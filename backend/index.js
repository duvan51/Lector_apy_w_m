...
app.put('/wocomerce/products', async (req, res) => {
 
  const jsonFilePath = './jsonActualizar/productos.json'; //datos desde el backend
 
 // const jsonData = req.body; // Datos enviados desde el frontend (JSON)
  ...
  try {
    ...
    const { data: allProducts } = await WooCommerce.get('products');
    
    
    // Itera a través de los datos de actualización y realiza las comparaciones y actualizaciones
    for (const updateItem of jsonData) {
      const productId = updateItem.id; // ID del producto del archivo JSON
      console.log(productId)
      // Busca el producto correspondiente en la lista de productos de WooCommerce
      const existingProduct = allProducts.find(product => product.id === productId);

      if (existingProduct) {
        // Si se encuentra un producto con el mismo ID, actualiza los datos
        const updatedData = { ...existingProduct, ...updateItem };
        await WooCommerce.put(`products/${productId}`, updatedData);
        console.log(`Producto actualizado con éxito: ${existingProduct.name}`);
      } else {
        console.log(`El producto con ID ${productId} no existe en WooCommerce. No se puede actualizar.`);
      }
    }

    res.json({ message: 'Productos actualizados con éxito en el servidor' });
  } catch (error) {
    console.error('Error al actualizar productos en el servidor:', error);
...
//wocomerce recibir el json
app.post('/api/upload-json', (req, res) => {
  const { json } = req.body; // Asume que el JSON se envía como parte del cuerpo de la solicitud
  // Puedes realizar validaciones y procesamiento adicional aquí
  res.status(200).json({ message: 'JSON cargado exitosamente' });
});
app.get('*', (req, res) => {
